...
/*
*	Plugin Name: Form To Questionair
*	Description: Plugin created to convert questionair form to CSV.
...
// create db table 
function create_form_to_questionair_tables () {
	global $wpdb;
	...
	register_activation_hook(__FILE__, 'create_form_to_questionair_tables');

	// Plugin Settings Page
	add_action('admin_menu', 'formQuestionair_settings');

	function formQuestionair_settings(){
		add_menu_page('Color',
						'form Questionair', 
						'manage_options',
						'formQuestionair',
						'form_to_questionair_settings', 
						'dashicons-clipboard', 
						'90');
...
function form_to_questionair_settings() {
	...
	// show quiz
		?>
	<br>
	   <a  <?php if (isset($_GET['hide_quiz'])){echo 'style="display: none;"';} ?> href="<?= admin_url(); ?>?page=formQuestionair&add_quiz" class="btn btn-default pull-right">Add Questionair</a>

	   <a style="display:<?php if (isset($_GET['hide_quiz'])){echo 'inline-block';}else { echo 'none';}?>" href="<?= admin_url(); ?>?page=formQuestionair" class="btn btn-default pull-">Home</a>
		...
		foreach( $results as $result ) {
			...
			<tr>
				...
				<td>
					...
					<a class="btn btn-default" href="<?= admin_url(); ?>?page=formQuestionair&view_qs_title=<?= $result->name; ?>&view_qs=<?= $result->id; ?>&hide_quiz=1">View MCQs</a>

					<!-- <a class="btn btn-default" href="<?= admin_url(); ?>?page=formQuestionair&view_qs_title=<?= $result->name; ?>&color_pattern_view=<?= $result->id; ?>&hide_quiz=1">View Color Response</a> -->

					<a class="btn btn-default" href="<?= admin_url(); ?>?page=formQuestionair&view_qs_title=<?= $result->name; ?>&edit_quiz=<?= $result->id; ?>&hide_quiz=1">Edit Quiz</a>

					<a class="btn btn-default" href="<?= admin_url(); ?>?page=formQuestionair&view_qs_title=<?= $result->name; ?>&delete_quiz=<?= $result->id; ?>" onclick="return confirm(' you want to delete?');">Delete Questionair</a>
				</td>
			</tr>
			<?php
	    }
	...
	<h1>Add Your First Questionair</h1>
	...
	if(isset($_GET['add_qs']) && !empty($_GET['add_qs'])){
		$add_qs = $_GET['add_qs'];
	?>
	<h2>Adding Question and Answers to Questionair # <?= $_GET['add_qs'] ?> <a href="<?= admin_url(); ?>?page=formQuestionair&view_qs_title=salman&view_qs=<?= $_GET['add_qs'] ?>&hide_quiz=1" class="btn btn-default pull-right">View All MCQs</a></h2>
	<form action="<?= admin_url(); ?>?page=formQuestionair&view_qs_title=salman&view_qs=<?= $_GET['add_qs'] ?>&hide_quiz=1" method="POST">
	  <div class="form-group">
	  ...
	  <div class="form-group">
	    ...
	    <div class="mark" style="opacity: 0; height: 0">
	...
	<div class="view_quiz_qns">
		...
		<h2>View All Questionair Questions and answer of Questionair(<span><?= $_GET['view_qs_title']; ?></span>) <a href="<?= admin_url(); ?>?page=formQuestionair&add_qs=<?= $_GET['view_qs'] ?>&hide_quiz=1" class="btn btn-default pull-right">Add more Questions</a>
	...
	if(!empty($results)){
	 ...
	 ?>
		<div class="view_qs">
			...
			<a href="<?= admin_url(); ?>?page=formQuestionair&view_qs_title=<?= $result -> question ; ?>&add_ans=<?= $result->id; ?>&hide_quiz=1&quiz_id=<?= $_GET['view_qs']?>" class="btn btn-primary">Add Answers</a>

			<a href="<?= admin_url(); ?>?page=formQuestionair&view_qs_title=<?= $_GET['view_qs_title']; ?>&edit_ques=<?= $result->id; ?>&hide_quiz=1&quiz_id=<?= $_GET['view_qs']?>" class="btn btn-primary">Edit</a>

			<a href="<?= admin_url(); ?>?page=formQuestionair&view_qs=<?= $_GET['view_qs']?>&view_qs_title=<?= $_GET['view_qs_title']; ?>&hide_quiz=1&delete_ques=<?= $result->id; ?>" onclick="return confirm(' Are you sure want to delete this Question?');" class="btn btn-primary">Delete</a></h3>
	...
	if($result_ans -> mark == 0) {
		...
		$mark = 'grey';
			 ...
			 ?>
				<li><?= $result_ans -> answer ?> <span style="display: none" class="mark color-<?= $mark; ?>"><i><?= $mark; ?></i> </span>  
				<a href="<?= admin_url(); ?>?page=formQuestionair&view_qs_title=<?= $_GET['view_qs_title']; ?>&edit_ans=<?= $result_ans->id; ?>&hide_quiz=1&quiz_id=<?= $_GET['view_qs']?>&question_title=<?= $result -> question ?>" class="btn btn-primary">Edit</a> 
				<a href="<?= admin_url(); ?>?page=formQuestionair&view_qs=<?= $_GET['view_qs']?>&view_qs_title=<?= $_GET['view_qs_title']; ?>&hide_quiz=1&delete_ans=<?= $result_ans->id; ?>" onclick="return confirm(' Are you sure want to delete this Answer?');" class="btn btn-primary">Delete</a></li>
	...
	<div class="view_quiz_qns">
		<h2>Adding color Responses to Questionair(<span><?= $_GET['view_qs_title']; ?></span>)</h2>

		<form action="<?= admin_url(); ?>?page=formQuestionair&view_qs_title=<?= $_GET['view_qs_title']; ?>&color_pattern_view=<?= $_GET['color_pattern']; ?>&hide_quiz=1" method="POST">
	...
	?>
	<div class="view_quiz_qns">
		<h2>Color Response of Questionair(<span><?= $_GET['view_qs_title']; ?></span>) <a class="btn btn-default pull-right" href="<?= admin_url(); ?>?page=formQuestionair&view_qs_title=<?= $_GET['view_qs_title']; ?>&color_pattern=<?= $_GET['color_pattern_view']; ?>&hide_quiz=1">Add Color Response</a></h2>
	<?php 
	...
	if($result_clr -> result_id == 0) {
		...
		$mark = 'grey';
			 ...
			 ?>
				...
				<a href="<?= admin_url(); ?>?page=formQuestionair&color_pattern_viewz=<?= $_GET['color_pattern_view']; ?>&view_qs_title=<?= $_GET['view_qs_title']; ?>&edit_res=<?= $result_clr->id; ?>&hide_quiz=1" class="btn btn-primary">Edit</a>

				<a href="<?= admin_url(); ?>?page=formQuestionair&view_qs_title=<?= $_GET['view_qs_title']; ?>&color_pattern_view=<?= $_GET['color_pattern_view']; ?>&hide_quiz=1&delete_res=<?= $result_clr->id; ?>" onclick="return confirm(' Are you sure want to delete this Answer?');" class="btn btn-primary">Delete</a>
	...
	<h2>Edit Question of Questionair(<span><?= $_GET['view_qs_title']; ?></span>)</h2>
	<form action="<?= admin_url(); ?>?page=formQuestionair&view_qs=<?= $_GET['quiz_id']?>&view_qs_title=<?= $_GET['view_qs_title']?>&hide_quiz=1" method="POST">
	  <div class="form-group">
	  <input type="hidden" name="id" value="<?= $results->id?>">
	...
	<form action="<?= admin_url(); ?>?page=formQuestionair&view_qs=<?= $_GET['quiz_id']?>&view_qs_title=<?= $_GET['view_qs_title']?>&hide_quiz=1" method="POST">
	  ...
	  <div class="form-group">
		...
		<select name="mark" style="opacity: 0; height: 0;" class="form-control" required id="mark"> 
		    	<option value="0" selected="selected">yellow</option> 
		    	<option value="1">purple</option> 
		    	<option value="2">green</option> 
		    	<option value="3">orange</option> 
	...
	if(isset($_GET['edit_res']) && !empty($_GET['edit_res'])){
				...
				$results = $wpdb->get_row($sql_get_quiz);
	?>
	<h2>Edit Response of Questionair(<span><?= $_GET['view_qs_title']; ?></span>)</h2>
		<form action="<?= admin_url(); ?>
		?page=formQuestionair&view_qs_title=<?= $_GET['view_qs_title']?>&color_pattern_view=<?= $_GET['color_pattern_viewz']?>&hide_quiz=1" method="POST">
...
function add_ajax_file(){
	wp_enqueue_script('form_to_questionair-ajax', plugins_url('/assets/js/ajax.js',__FILE__), array('jquery'), true);
	wp_localize_script('form_to_questionair-ajax', 'form_to_questionair_ajax_url', array(
			'ajax_url'=> admin_url('admin-ajax.php')
		));
}

function load_custom_plugin_scripts_formQuestionair(){
...
function load_custom_plugin_scripts2_formQuestionair(){
	wp_register_style('main_css', plugin_dir_url(__FILE__).'/assets/css/main.css', false, '1.0.0');
	wp_enqueue_style('main_css');
}

add_action('admin_enqueue_scripts', 'load_custom_plugin_scripts_formQuestionair');
add_action('admin_enqueue_scripts', 'load_custom_plugin_scripts2_formQuestionair');
add_action('wp_enqueue_scripts', 'add_ajax_file');

add_action('wp_ajax_form_to_questionair_ajax_function', 'form_to_questionair_ajax_function');
add_action('wp_ajax_nopriv_form_to_questionair_ajax_function', 'form_to_questionair_ajax_function');


function form_to_questionair_ajax_function(){
	global $current_user;
	global $wpdb;
	$user_answers = $wpdb->prefix . 'user_answers';
